    ...
    def _assisted_decoding(
        ...
        **sample** (depending on `do_sample`), assisted by candidate sequences. Assisted generation is an example of a
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # MIGHT NOT WORK
            # outputs.past_key_values = _crop_past_key_values(self, outputs.past_key_values, new_cache_size) # Transformers depreciated this, manual method below:
            if model_kwargs.get("past_key_values", None) is not None:
                model_kwargs["past_key_values"] = tuple(
                    tuple(
                        past_key_value[:, :, :new_cache_size, :] if past_key_value is not None else None
                        for past_key_value in layer_past_key_values
                    )
                    for layer_past_key_values in model_kwargs["past_key_values"]
                )
